"""
AstrBot Plugin - B站AI伴侣 1.0.0
自动回复评论、好感度、记忆、心情、用户画像、主动视频、动态发布，与 memory_companion 双轨记忆同步。
...
from .core import (
    UtilsMixin, LLMMixin, VisionMixin, MemoryMixin, MemorySyncMixin,
...
_BILIBOT_MAIN_TASK_NAME = "astrbot_plugin_bili_ai_companion.main_loop"


def get_bili_ai_companion_api():
...
@register("astrbot_plugin_bili_ai_companion", "trae", "B站AI伴侣 — 评论自动回复、好感度、记忆、心情、主动视频、动态发布、memory_companion双轨同步", "1.0.0", "")
class BiliAICompanion(Star, UtilsMixin, LLMMixin, VisionMixin, MemoryMixin, MemorySyncMixin, AffectionMixin, PersonalityMixin, BilibiliAPIMixin, BangumiMixin, WebSearchMixin, VideoMixin, ReplyMixin, ProactiveMixin, DynamicMixin, ScheduleMixin, WeeklySummaryMixin, ShareMixin, PrivateMessageMixin):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        logger.info(f"[BiliCompanion] LLM工具已精简注册: {len(llm_tools)} 个 ({tool_names})")

        # ── memory_companion 双轨同步状态 ──
        self._memory_sync_state = self._load_sync_state()

        # ── 拓展页 API 注册 ──
        self.page_api = None
        self._register_page_api_if_available()

    def _register_page_api_if_available(self):
        """注册拓展页 Web API（供 pages/stats WebUI 调用）。"""
        ...
        try:
            ...
            self.page_api = None
            logger.warning(f"[BiliCompanion] 拓展页 API 注册失败: {exc}", exc_info=True)

    async def initialize(self):
        """Start background work only after AstrBot has formally activated the plugin."""
        if not self._has_cookie():
            logger.warning("[BiliCompanion] Cookie未配置，后台任务未启动")
            return
        valid, msg = await self.check_cookie()
        if valid:
            await self._start_bot()
            logger.info("[BiliCompanion] 自动启动")
        elif "检查失败" in str(msg):
            # 网络暂时不可用 ≠ Cookie 失效（如开机自启时网络未就绪），仍启动主循环靠周期检查兜底
            logger.warning(f"[BiliCompanion] Cookie 检查暂时失败（{msg}），仍启动后台任务等待网络恢复")
            await self._start_bot()
        else:
            logger.warning("[BiliCompanion] Cookie无效")
    ...
    async def _start_bot(self):
        async with self._start_lock:
            ...
            if stale_tasks:
                ...
                logger.warning(
                    f"[BiliCompanion] 已清理 {len(stale_tasks)} 个热重载残留主循环"
                )
            ...
            if self.config.get("ENABLE_PRIVATE_MESSAGES", False):
                ...
                if isinstance(private_state, dict) and private_state.get("initialized"):
                    ...
                    logger.info(
                        f"[BiliCompanion] 私信监听将在 {startup_delay} 秒启动，"
                        "避免重载后立即重试接口"
                    )
            self._running = True
    ...
    async def _stop_bot(self):
        ...
        if self._consolidation_task and not self._consolidation_task.done():
            ...
            self._consolidation_task = None
        logger.info("[BiliCompanion] 停止")

    async def _run_consolidation_safe(self):
        """安全执行日终清算，捕获所有异常。"""
        try:
            logger.info("[BiliCompanion] 🌙 开始日终清算...")
            summary = await self._consolidation.run_daily()
            logger.info(f"[BiliCompanion] 🌙 日终清算完成:\n{summary}")
        except Exception as e:
            logger.error(f"[BiliCompanion] 日终清算异常: {e}", exc_info=True)

    async def _main_loop(self):
        logger.info("[BiliCompanion] 主循环开始")
        while self._running:
            try:
                ...
                if in_sleep:
                    ...
                    # ── 周总结：日终清算已完成且不在运行时触发 ──
                        ...
                        try:
                            ...
                            logger.error(f"[BiliCompanion] 周总结调度异常: {e}")
                ...
                if self.config.get("ENABLE_PROACTIVE", False):
                    ...
                    if sched.get("date") != today_str:
                        ...
                        logger.info(f"[BiliCompanion] 📅 新的一天！主动视频时间：{[f'{ph}:{pm:02d}' for ph,pm in self._proactive_times]}")
                    ...
                    for ph, pm in self._proactive_times:
                        ...
                        if key not in self._proactive_triggered and (now_dt.hour > ph or (now_dt.hour == ph and now_dt.minute >= pm)):
                            if self._proactive_task is None or self._proactive_task.done():
                                ...
                                logger.info(f"[BiliCompanion] 🎯 触发主动视频（{key}）")
                                break
                if self.config.get("ENABLE_DYNAMIC", False):
                    ...
                    if sched.get("date") != today_str:
                        ...
                        logger.info(f"[BiliCompanion] 📅 动态时间：{[f'{dh}:{dm:02d}' for dh,dm in self._dynamic_times]}")
                    ...
                    for dh, dm in self._dynamic_times:
                        ...
                        if key not in self._dynamic_triggered and (now_dt.hour > dh or (now_dt.hour == dh and now_dt.minute >= dm)):
                            if self._dynamic_task is None or self._dynamic_task.done():
                                ...
                                logger.info(f"[BiliCompanion] 📢 触发动态发布（{key}）")
                                break
                if self.config.get("ENABLE_PRIVATE_MESSAGES", False):
                ...
                if self.config.get("ENABLE_BANGUMI", False) and self.config.get("BANGUMI_PROACTIVE", False):
                    ...
                    if bsched.get("date") != today_str:
                        ...
                        logger.info(f"[BiliCompanion] 📅 番剧时间：{[f'{bh}:{bm:02d}' for bh,bm in self._bangumi_times]}")
                        ...
                        if not self._bangumi_update_checked:
                            for bh, bm in self._bangumi_times:
                                ...
                                if key not in self._bangumi_triggered and (now_dt.hour > bh or (now_dt.hour == bh and now_dt.minute >= bm)):
                                    ...
                                    self._save_json(PROACTIVE_TRIGGER_LOG_FILE, trigger_log[-200:])
                                    logger.info(f"[BiliCompanion] 📺 触发每日追番更新检查（{key}）")
                                    break
                        else:
                            for bh, bm in self._bangumi_times:
                                key = f"{bh}:{bm:02d}"
                                if key not in self._bangumi_triggered and (now_dt.hour > bh or (now_dt.hour == bh and now_dt.minute >= bm)):
                                    ...
                                    self._save_json(PROACTIVE_TRIGGER_LOG_FILE, trigger_log[-200:])
                                    logger.info(f"[BiliCompanion] 🎬 触发主动看番（{key}）")
                                    break
                # 特别关注定时巡视
                if self.config.get("SPECIAL_FOLLOW_ENABLED", False):
                    ...
                    if sfsched.get("date") != today_str:
                        ...
                        logger.info(f"[BiliCompanion] 📅 特关时间：{[f'{sh}:{sm:02d}' for sh,sm in self._special_follow_times]}")
                    ...
                    for sh, sm in self._special_follow_times:
                        ...
                        if key not in self._special_follow_triggered and (now_dt.hour > sh or (now_dt.hour == sh and now_dt.minute >= sm)):
                            if self._special_follow_task is None or self._special_follow_task.done():
                                ...
                                self._save_json(PROACTIVE_TRIGGER_LOG_FILE, trigger_log[-200:])
                                logger.info(f"[BiliCompanion] ⭐ 触发特别关注巡视（{key}）")
                                break
                await asyncio.sleep(self.config.get("POLL_INTERVAL", 20))
            except asyncio.CancelledError:
                break
            except Exception as e:
                logger.error(f"[BiliCompanion] 主循环出错: {e}\n{traceback.format_exc()}")
    ...
    async def _check_and_refresh_cookie(self):
        ...
        if valid:
            logger.info(f"[BiliCompanion] Cookie OK: {info}")
            return
        logger.warning(f"[BiliCompanion] Cookie 失效: {info}")
        if self.config.get("COOKIE_AUTO_REFRESH", True):
            ok, msg = await self.refresh_cookie()
            logger.info(f"[BiliCompanion] 刷新{'成功' if ok else '失败'}: {msg}")
    ...
    async def cmd_status(self, event: AstrMessageEvent):
        ...
        lines = [
            f"📺 BiliCompanion 1.0.0 状态","━━━━━━━━━━━━",f"🍪 {info}",
            ...
            f"🔗 记忆模式: {self._memory_sync_mode_label()} | companion: {'✅' if self._get_memory_companion() else '❌'}",
    ...
    @filter.command("bili同步")
    async def cmd_memory_sync(self, event: AstrMessageEvent):
        """查看记忆模式与memory_companion的同步状态。"""
        companion = self._get_memory_companion()
        mode = self._memory_sync_mode()
        mode_label = self._memory_sync_mode_label()
        sync_enabled = self._is_sync_to_companion_enabled()
        local_writable = self._is_local_memory_writable()
        state = self._load_sync_state()
        lines = [
            "🧠 记忆系统状态",
            "━━━━━━━━━━━━",
            f"当前模式: {mode_label}（{mode}）",
            f"本地B站记忆写入: {'✅' if local_writable else '❌（仅缓存）'}",
            f"同步到memory_companion: {'✅' if sync_enabled else '❌'}",
            f"companion插件: {'✅ 已找到' if companion else '❌ 未找到'}",
            f"已同步事件数: {state.get('synced_count', 0)}",
            f"上次同步: {state.get('last_sync', '从未')}",
        ]
        if mode == "standalone":
            lines.append("状态: 📦 独立模式，仅使用本地B站记忆")
        elif mode == "companion":
            if companion:
                lines.append("状态: 🐝 伴侣模式，优先写入memory_companion，本地仅缓存")
            else:
                lines.append("状态: ⚠️ 伴侣模式但未找到memory_companion，将退回本地记忆")
        else:  # dual
            if companion:
                lines.append("状态: ✅ 双轨记忆同步就绪")
            else:
                lines.append("状态: ⚠️ 双轨模式但未检测到memory_companion，仅本地记忆")
        yield event.plain_result("\n".join(lines))
    ...
    async def cmd_cleanup(self, event: AstrMessageEvent):
        ...
        if full_clean:
            ...
            msg_lines.append("")
            msg_lines.append("💡 提示：如需完全重置，手动删除 plugin_data/astrbot_plugin_bili_ai_companion 目录")
        else:
            msg_lines.append("")
    ...
    async def inject_bili_memory(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        try:
            ...
            if self._inject_recent_group_share_into_request(event, req):
                logger.debug("[BiliCompanion] recent group share context injected before user message")
            qq_id = str(event.get_sender_id())
            bindings = self._load_json(BINDING_FILE, {})
            if qq_id not in bindings:
                return
            ...
            logger.debug(f"[BiliCompanion] QQ->Bili binding context injected before user message: uid={bili_uid}")
        except Exception as e:
            logger.error(f"[BiliCompanion] 记忆注入失败: {e}")
